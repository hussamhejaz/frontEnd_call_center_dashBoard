...
const Users = () => {
  ...
  const filteredUsers = usersData.filter(user => {
    ...
    const matchesSearch = user.FirstName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                          ...
                          user.PhoneNumber?.toLowerCase().includes(searchTerm.toLowerCase()); // Added phone number search
    ...
    <div className="users-container-wrapper">
      <div className="users-container">
        ...
        <div className="users-controls">
          ...
          />
          <select id="genderFilter" onChange={(e) => setSelectedGender(e.target.value)} className="users-filter">
            <option value="All">All Genders</option>
            <option value="Male">Male</option>
            <option value="Female">Female</option>
          </select>
          <select id="accountTypeFilter" onChange={(e) => setSelectedAccountType(e.target.value)} className="users-filter">
        ...
        <div className="users-list">
          {filteredUsers.map(user => (
            <div key={user.id} className={`user-card ${getAccountTypeClass(user.accountType)}`} onClick={() => handleUserClick(user.id)}>
              <div className="user-card-inner">
                <div className="user-info">
                  <h3 className="user-name">{`${user.FirstName} ${user.LastName}`}</h3>
                  <p className="user-email"><strong>Email:</strong> {user.Email}</p>
                  <p className="user-phone"><strong>Phone:</strong> {user.PhoneNumber}</p>
                  <p className="user-gender"><strong>Gender:</strong> {user.Gender}</p>
